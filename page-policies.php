<?php
/**
 * Template for policies content
 *
 * @package WordPress
 * @subpackage Twenty_Eleven Child
 * @since Twenty Eleven 1.0
 */

get_header(); ?>

		<div id="primary">
			<div id="content" role="main">

				<div class="container">
					<div class="twelve columns"><h1 class="text-dashed center"><?php echo get_the_title(); ?></h1></div>

					<div class="twelve columns">
						<h2>Order Policies</h2>
						<ul class="ulp">
							<li>
								Cake orders will only be processed when submitted a week earlier or at least two days earlier.
							</li>
							<li>
								Order will not be processed unless deposit is made.
							</li>
							<li>
								We want you to be satisfied with your purchase! If for some reason your purchase does not delight you, please contact us! 
							</li>								
						</ul>
						<h2>Return Policies</h2>
						<ul class="ulp">
							<li>
								We do not accept returns on internet or retail sales unless we have made an error. 
							</li>
							<li>
								We will work with you to correct your order and make things right as quickly as possible. 
							</li>
							<li>
							 	We want you to be satisfied with your purchase. 	
							</li>	
							<li>
								If we have made a mistake, please call us and we will do everything we can to correct any errors. 
							</li>	
							<li>
								WE WANT TO EARN YOUR BUSINESS!
							</li>								
						</ul>
						<h2>Refund Policies</h2>
						<ul class="ulp">
							<li>
								We offer refunds on retail and internet sales if we have made an error in our shipment/delivery or if our products arrive in an unsatisfactory condition. 
							</li>
							<li>
								We will work with you to correct your order and make things right as quickly as possible. 
							</li>
							<li>
								We do not, however offer refunds on orders that have not been picked up. 
							</li>	
							<li>
								We want you to be satisfied with your purchase. If we have made a mistake, please call us and we will do everything we can to correct any errors.
							</li>								
						</ul>
						<h2>Cancellation Policies</h2>
						<ul class="ulp">
							<li>
								Please know that your satisfaction is our ultimate goal. Please contact us about any aspect of your order and we will do our best to ensure your satisfaction. 
							</li>
							<li>
								MANGKIU Home Sweet Home does not allow cancellations on internet sales because we begin processing those orders immediately. 
							</li>
							<li>
								Any Wedding Cakes, Groom’s cakes, three-dimensional carved cakes, or other tiered cakes cancelled less than 90 days prior to the scheduled date of the event, will receive NO refunds on any payments made in securing the cakes, including the initial deposit or subsequent payments, up to and including final payments. Cakes cancelled more than 90 days before the wedding date will receive a full refund. 
							</li>						
						</ul>
						<div style="width:90%; margin: 0 auto">
							Cake Changes:
							<ul class="ulp">	
								<li>
									Changes to your cake size, description, flavors, etc. will be accepted until 2 weeks prior to the wedding date for wedding cakes and 3 days for others. After that lockout date we cannot accept any changes. 
								</li>
								<li>
									If you insist on changes after the lockout date, you will incur additional non-refundable charges. 
								</li>						
							</ul>
						</div>
						<h2>Matching Colors</h2>
						<ul class="ulp">
							<li>
								We make every possible effort to match your wedding colors, however, to get as close as possible you must provide us with color swatch. 
							</li>
							<li>
								Please realize that when working with food grade dyes, perfect color matches are sometimes impossible. 
							</li>
							<li>
								Dark colored icings have a tendency to darken further with time, humidity, and heat. The darker colors may also stain teeth and will have some residual flavor.
							</li>								
						</ul>
						<h2>Email Communications</h2>
						<ul class="ulp">
							<li>
								We send email confirmations of your online order to you. 
							</li>
							<li>
								We may also need to contact you by email if we have any questions as we’re processing your order. 
							</li>
							<li>
								When you order online, your name is added to our email list. This list is used for occasional emails regarding our web site, new product offerings, seasonal offerings, and special promotions.
							</li>								
						</ul>
						<h2>Order Modifications</h2>
						<ul class="ulp">
							<li>
								If you need to modify an order after it’s been placed, we can accommodate your request as long as we have not produced or shipped your order.
							</li>
							<li>
								All requests must be made by phone or email mangkiuhomesweethome@hotmail.com. 
							</li>
							<li>
								Please be sure to include your order number with your request. We will confirm the change by phone, or email.
							</li>							
						</ul>
						<h2>Customization</h2>
						<ul class="ulp">
							<li>
								We offer customization of products so if you wish to speak with someone regarding your specific requirements, please call us weekdays from 9 a.m. to 5 p.m. or email mangkiuhomesweethome@hotmail.com. 
							</li>
							<li>
								Please be sure to include a daytime telephone number in any email correspondence.
							</li>								
						</ul>
						<h2>Product Guarantee</h2>
						<ul class="ulp">
							<li>
								If you are dissatisfied with your purchase, please contact us weekdays between 9 A.M. and 5 P.M.. 
							</li>
							<li>
								We request that you retain all remaining product and packaging materials so we can best assist you. 
							</li>
							<li>
								If your product arrives damaged, we request that you contact us within 24 hours of receiving your package.
							</li>								
						</ul>
						<h2>Food Allergens</h2>
						<ul class="ulp">
							<li>
								Please inform us of any food allergies before we process your order.
							</li>
							<li>
								Given the nature of our production process, please be aware that any product could contain traces of ingredients used in the production of other products (i.e. there could be cross-contamination).
							</li>
							<li>
								These ingredients include but are not limited to tree nuts, peanuts, chocolate, soy, and wheat flour (gluten).
							</li>								
						</ul>
					</div>
				</div>

			</div><!-- #content -->
		</div><!-- #primary -->

<?php get_sidebar(); ?>
<?php get_footer(); ?>